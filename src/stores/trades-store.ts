import { useMemo } from 'react'
  ...
  export function toApiTrade(t: Trade, accountId: string, accountName: string) {
    return {
      id: t.id,
  ...
  export const useTradesStore = create<TradesState>()((set, get) => ({
    ...
    _hydrate: (trades) => set({ trades, isLoading: false }),
    ...
    addTradesForAccount: (accountId, accountName, incoming) => {
      ...
      for (const t of incoming) {
        if (existingIdsForAccount.has(t.id) || seen.has(t.id)) { duplicates++; continue }
        seen.add(t.id)
      ...
      for (const t of fresh) {
        apiFetch('/api/trades', 'POST', toApiTrade(t, accountId, accountName))
      }
      return { added: fresh.length, duplicates }
    },

    updateTrade: (trade) => {
      set((s) => ({ trades: s.trades.map((t) => t.id === trade.id ? trade : t) }))
      apiFetch(
        `/api/trades/${trade.id}`,
        'PUT',
        toApiTrade(trade, trade.accountId ?? '', trade.account)
      )
    },
    ...
    removeTrade: (id) => {
      ...
      if (removed) apiFetch(`/api/trades/${id}`, 'DELETE')
      return removed
    },
    ...
    removeTrades: (ids) => {
      ...
      const before = get().trades.length
      set((s) => ({ trades: s.trades.filter((t) => !idSet.has(t.id)) }))
      const count = before - get().trades.length
      if (count > 0) apiFetch('/api/trades/bulk-delete', 'POST', { ids })
      return count
    },

    clearTradesForAccount: (accountId) => {
      const before = get().trades.length
      set((s) => ({ trades: s.trades.filter((t) => t.accountId !== accountId) }))
      const count = before - get().trades.length
      if (count > 0) apiFetch('/api/trades/bulk-delete', 'POST', { ids: [], account_id: accountId })
      return count
    },

    removeAccount: (accountId) => {
      set((s) => ({ trades: s.trades.filter((t) => t.accountId !== accountId) }))
      apiFetch('/api/trades/bulk-delete', 'POST', { ids: [], account_id: accountId })
    },

    reset: () => set({ trades: [], isLoading: false }),
  }))
